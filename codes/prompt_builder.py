"""
Prompt builder — build full prompt strings from YAML prompt config.

Role: Turn prompt_config entries (e.g. intake_assistant_prompt, summary_assistant_prompt)
into single prompt strings. Config keys: role, instruction (required), context,
output_constraints, style_or_tone, output_format, examples, goal. Optional input_data
is appended as <<<BEGIN CONTENT>>> ... <<<END CONTENT>>>. Used by nodes that call the LLM
(intake, summary) via build_prompt_from_config. Do not hardcode prompts in nodes.
"""
